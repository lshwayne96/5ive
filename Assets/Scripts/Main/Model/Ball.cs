using System; using UnityEngine;  /// <summary> /// This script represents a ball and its interactions. /// </summary> public class Ball : RestorableMonoBehaviour {
    ...
    public float speed = 10f;      private Transform playerTf;      public bool PlayerHasBall { get; private set; }     public bool IsPlayerWithinRange { get; private set; }      void Start() {
        playerTf = GameObject.FindWithTag(Tags.Player).transform;     }      void Update() {
        ...
        if (CanPickUpBall()) {             PickUpBall();         } else if (CanDropBall()) {
            DropBall();         }     }      private void OnTriggerEnter2D(Collider2D collision) {         if (collision.gameObject.CompareTag(Tags.Player)) {             IsPlayerWithinRange = true;         }     }      private void OnTriggerExit2D(Collider2D collision) {         if (collision.gameObject.CompareTag(Tags.Player)) {             IsPlayerWithinRange = false;         }     }      private bool CanPickUpBall() {         return IsPlayerWithinRange && Input.GetKeyDown(KeyCode.E) && !PlayerHasBall && !PauseLevel.IsPaused;     }      private bool CanDropBall() {         return Input.GetKeyDown(KeyCode.E) && PlayerHasBall && !PauseLevel.IsPaused;     }      private void PickUpBall() {         transform.position = playerTf.position;         GetComponent<Rigidbody2D>().gravityScale = 0f;         PlayerHasBall = true;     }      private void DropBall() {         GetComponent<Rigidbody2D>().gravityScale = 1f;         PlayerHasBall = false;     }      public override Data Save() {         return new BallData(this);     }      public override void RestoreWith(Data data) {         BallData ballData = (BallData) data;         GetComponent<Rigidbody2D>().velocity = ballData.PrevVelocity;         transform.position = ballData.PrevPosition;         // Restore ball camera         transform = GetComponent<RoomDetector>().GetCurrentRoom();         PlayerHasBall = ballData.PlayerHasBall;         IsPlayerWithinRange = ballData.IsPlayerWithinRange;     }

	/// <summary> 	/// This class represents the data of a ball. 	/// </summary> 	/// <remarks> 	/// It is used to restore the ball to its previous state. 	/// </remarks> 	/// The data includes: 	/// <list type="number"> 	/// <item>The last position</item> 	/// <item>The last velocity</item> 	/// </list> 	[Serializable]
	public class BallData : Data {
		...
		/// <summary> 		/// Initializes a new instance of the <see cref="T:BallData"/> class. 		/// </summary> 		/// <param name="ball">Ball.</param> 	    public BallData(Ball ball) {
