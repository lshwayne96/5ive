/*  * This script enables interaction with the  * attached gameObject, namely a ball gameObject.  * Interactions includes picking and dropping the ball.  */  using UnityEngine;  public class Ball : MonoBehaviour {
    ...
    public float speed = 10f;      private GameObject player;     private Transform playerTf;      private bool playerHasBall;     private bool canPickUpBall;      private Vector2 preVelocity;      void Start() {         player = GameObject.FindWithTag("Player");         playerTf = player.transform;     }      void Update() {
        ...
        if (CanPickUpBall()) {             PickUpBall();         }

        // Enable the ball to be dropped anytime
        if (CanDropBall()) {             DropBall();         }     }      private void OnTriggerEnter2D(Collider2D collision) {         if (collision.gameObject.CompareTag("Player")) {             canPickUpBall = true;         }     }      private void OnTriggerExit2D(Collider2D collision) {         if (collision.gameObject.CompareTag("Player")) {             canPickUpBall = false;         }     }      private bool CanPickUpBall() {         return canPickUpBall && Input.GetKeyDown(KeyCode.G) && !playerHasBall && !PauseLevel.isPaused;     }      private bool CanDropBall() {         return Input.GetKeyDown(KeyCode.H) && playerHasBall && !PauseLevel.isPaused;     }      private void PickUpBall() {         transform.position = playerTf.position;         GetComponent<Rigidbody2D>().gravityScale = 0f;         playerHasBall = true;     }      private void DropBall() {         GetComponent<Rigidbody2D>().gravityScale = 1f;         playerHasBall = false;     } }  